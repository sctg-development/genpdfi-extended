...
"#),
        ...
        for (idx, (title, diagram)) in diagrams.iter().enumerate() {
            ...
            match elements::Mermaid::render_svg(diagram) {
                ...
                Err(e) => {
                    // Add a note to the document and continue with the next diagrams.
                    println!("✗ Diagram {} failed to compile: {}", idx + 1, e);
                    doc.push(elements::Paragraph::new(format!("Diagram {} failed to compile: {}", idx + 1, e)));
                }
            }
        }
